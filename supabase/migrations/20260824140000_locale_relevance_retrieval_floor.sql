-- Retrieval had no notion of whether a catalog row belongs to the query's
-- market, so a Turkish sentence competed against 41,503 rows with zero Turkey
-- relevance. "kremalı tavuklu makarna" reached an Open Food Facts row named
-- "Kremal" (Romanian barcode 5941664008870, turkey_relevance_score 30) and the
-- real Makarna / Tavuk / Krema rows lost to it.
--
-- The signal was already imported: `import-lean-production-postgres.ts` writes
-- tier, turkey_relevance_score and english_relevance_score into
-- public.foods.metadata. Nothing read them. Measured over the 60,000-row lean
-- catalog, turkey_relevance_score is below 50 for 100% of off_en_global,
-- usda_branded_quality and quality_global rows, and for half of tr_branded.
--
-- The floor value lives in the caller (ANALYSIS_LOCALE_RELEVANCE_FLOOR = 40),
-- chosen at the gap in the measured distribution: tr_branded uses 25 and 30
-- and then jumps to 40, so cutting at 40 drops the 1,850 weakest rows —
-- "Kremal" among them — without taking a real product like Nutella at 45.
--
-- generic_core is deliberately exempt. Those are the USDA generic foods
-- (Chicken, Egg, ...) that score 0 on Turkey relevance because they are not
-- Turkish *products*, while still being the correct fallback for a food with no
-- Turkish row. Excluding them would remove real answers, not noise.
--
-- The floor is opt-in per call. Manual catalog search must keep reaching every
-- row: a user typing a specific imported product is not making a mistake. Only
-- the analyze path, where the query is a meal sentence rather than a product
-- name, passes a floor.
