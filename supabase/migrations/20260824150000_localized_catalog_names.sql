-- Half the catalog answers only to English. `translate-catalog-names-postgres.ts`
-- writes a Turkish name into foods.metadata.canonical_name_tr for the ~13,300
-- USDA rows whose only name was English; this is the read side of that.
--
-- Retrieval already reached those rows — the locale-relevance floor exempts
-- generic_core precisely so it can — but it handed back "Home recipe pasta with
-- cream sauce and meat" and the app put that in front of a Turkish user, in a
-- sheet that asked them to *choose* between six of them. The catalog name is
-- still the provenance; it is no longer the label.
--
-- Falls back to canonical_name whenever no translation exists, so this is safe
-- to apply before, during, or after the translation run.

create or replace function public.localized_name(
...
as $$
  select case
    when p_locale = 'tr-TR'
      -- COALESCE and NULLIF are SQL grammar, not functions, so they cannot be
      -- schema-qualified — and they need no qualification, since an empty
      -- search_path cannot shadow them.
      then coalesce(
        nullif(pg_catalog.btrim(p_metadata ->> 'canonical_name_tr'), ''),
        p_canonical_name
      )
    else p_canonical_name
  end
$$;

revoke all on function public.localized_name(text, jsonb, text) from public, anon;
grant execute on function public.localized_name(text, jsonb, text) to authenticated, service_role;
