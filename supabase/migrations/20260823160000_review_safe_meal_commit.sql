-- Review-safe analyzed-meal commit.
--
-- The original proposal remains grounded by analysis_candidates. A user may
-- explicitly choose another active catalog food or add a new catalog item;
-- nutrition is always re-read from foods and corrections remain auditable.

alter table public.analysis_runs
  add column if not exists image_path text;

alter table public.analysis_runs
  drop constraint if exists analysis_runs_image_path_format;
alter table public.analysis_runs
  add constraint analysis_runs_image_path_format check (
    image_path is null or image_path ~ '^[0-9a-f-]{36}/[0-9a-f-]{36}/source\.(jpg|png|webp)$'
  );

comment on column public.analysis_runs.image_path is
  'Private meal-photos object path captured during analysis and copied to the committed meal.';
...
begin
  ...
  loop
    if nullif(v_item ->> 'item_id', '') is null or
       nullif(v_item ->> 'item_key', '') is null or
       nullif(v_item ->> 'food_id', '') is null then
      raise exception using errcode = '22023', message = 'item_id, item_key and food_id are required';
    end if;
    begin
    ...
    exception when invalid_text_representation then
      raise exception using errcode = '22023', message = 'item_id, food_id and grams must be valid';
    end;
    ...
    if v_manual_addition or v_food_changed then
      begin
...
comment on function public.commit_analyzed_meal(uuid, uuid, uuid, text, timestamptz, jsonb) is
  'Commits grounded, variant, and manual review items using active catalog nutrition and client-stable item UUIDs.';
