...
import { buildEmailDeliveries, renderAuthEmail } from '../../src/routes/send-email/email.ts'

test('renders a branded OTP email with plain-text fallback', () => {
...
test('builds one delivery for passwordless sign in', () => {
...
test('builds both secure email-change deliveries with the correct tokens', () => {
  const deliveries = buildEmailDeliveries({
    user: {
      id: 'user-id',
...
test('escapes token content before rendering HTML', () => {
