...
import { EstimateRefusalError, estimateUnmatchedFoods } from '../../src/routes/analyze-meal/estimate.ts'
...
test('estimator accepts a bounds-checked, Atwater-consistent estimate', async () => {
...
test('estimator rejects Atwater-inconsistent macros', async () => {
  const result = await estimateUnmatchedFoods({
    ...baseOptions,
    maxAttempts: 1,
    fetcher: (() =>
...
test('estimator drops texts outside the request and clamps grams', async () => {
  const result = await estimateUnmatchedFoods({
    ...baseOptions,
    foods: ['ayran', 'menemen'],
    maxAttempts: 1,
...
test('estimator surfaces model refusal without retrying', async () => {
  let calls = 0
  await assertRejects(
    () =>
