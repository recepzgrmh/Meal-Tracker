...
} from '../../src/routes/analyze-meal/candidate-selector.ts'
...
test('selector retries 429 and accepts only allow-listed candidate IDs', async () => {
  let calls = 0
...
test('selector drops IDs outside the server allow-list', async () => {
  const result = await selectCatalogCandidates({
    apiKey: 'test',
    locale: 'tr-TR',
    input: 'uydurma',
...
test('selector surfaces model refusal for deterministic fallback', async () => {
  await assertRejects(
    () =>
      selectCatalogCandidates({
        apiKey: 'test',
...
test('a rejected request carries the provider explanation', async () => {
...
test('provider error detail prefers the message and stays bounded', () => {
