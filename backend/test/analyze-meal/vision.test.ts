...
import {
  ...
  VisionProviderError,
  VisionRefusalError,
} from '../../src/routes/analyze-meal/vision.ts'
...
test('vision retries 429 and records the successful attempt', async () => {
  let calls = 0
...
test('vision retries timeout and surfaces a typed timeout failure', async () => {
  let calls = 0
  await assertRejects(
    () =>
      requestVisionExtraction({
...
test('vision refusal is not retried', async () => {
  let calls = 0
  await assertRejects(
    () =>
      requestVisionExtraction({
...
test('vision success with zero foods is not retried and is not a provider failure', async () => {
  let calls = 0
  const extractor = () =>
    requestVisionExtraction({
      ...baseOptions,
...
test('mixed input safely falls back to text when vision fails', async () => {
...
test('photo-only input preserves provider failure for retryable API response', async () => {
  await assertRejects(
    () =>
      extractVisionWithTextFallback(
        ' ',
