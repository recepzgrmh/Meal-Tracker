...
// analyze-meal no longer lives behind the Supabase functions gateway, so its
// host is configured separately. Storage uploads and the analysis_runs readback
// below still go to Supabase, which is why both URLs are needed.
const backendUrl = requiredEnv('EVAL_BACKEND_URL').replace(/\/$/u, '')
const publishableKey = requiredEnv('EVAL_SUPABASE_PUBLISHABLE_KEY')
const userJwt = requiredEnv('EVAL_USER_JWT')
if (process.env.LIVE_EVAL_ACK !== 'I_ACCEPT_PROVIDER_COST') {
  throw new Error('Set LIVE_EVAL_ACK=I_ACCEPT_PROVIDER_COST to run paid provider evals')
}
const maxCases = Math.min(100, Math.max(1, Number(process.env.EVAL_MAX_CASES ?? 20)))
...
async function uploadPhoto(imagePath: string, userId: string, requestId: string) {
  const file = await readFile(new URL(imagePath, root))
