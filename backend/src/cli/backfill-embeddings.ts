...
const batchSize = Math.max(1, Math.min(500, Number(process.env.BACKFILL_BATCH_SIZE ?? 500)))
...
const ROUND_TIMEOUT_MS = Math.max(
  30_000,
  Number(process.env.BACKFILL_ROUND_TIMEOUT_MS ?? 120_000),
)
...
while (idleRounds < 2) {
  // Annotated rather than inferred: the only assignment sits inside the try, so
  // TypeScript cannot narrow it away from unknown at the use sites below.
  let result: CatalogBackfillResult
  try {
    ...
    // thousands of embedding vectors over PostgREST looks like; they are
