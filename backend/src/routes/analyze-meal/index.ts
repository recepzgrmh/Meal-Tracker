...
import { errorResponse, jsonHeaders, jsonResponse, redactedLog } from '../../shared/http.ts'
import {
  aliasLookupKeys,
...
import { sha256 } from '../../shared/embeddings.ts'
...
export async function handleAnalyzeMeal(
  ...
  try {
    ...
    const visionAnalysis = vision
      ? applyVisionEvidence(
        analyzeDeterministically(vision.normalizedDescription, catalog, body.locale ?? 'tr-TR'),
        vision.foods,
      )
      : null
...
async function loadCatalog(
  ...
  const lookupKeys = aliasLookupKeys(texts, locale)
...
async function resolveTextAnalysis(
  ...
  const direct = analyzeDeterministically(input.input, input.catalog, input.locale)
  const apiKey = process.env.OPENAI_API_KEY?.trim()
  // Leftovers were the only reason to spend a model call, which assumed that
  // covering the sentence meant understanding it. A matcher can also cover it by
  // cutting one dish into pieces that each happen to be a catalog row, and that
  // reads as full confidence while being the worst answer available. So a
  // fragmented cover is treated exactly like leftover text: it goes to the
  // model, whose prompt already knows a described dish is one item.
  const explained = direct.unmatchedText.length === 0 && !direct.fragmentedPhrase
  if (!input.input.trim() || explained || !apiKey) {
    return {
      analysis: direct,
      extraction: null,
  ...
  for (const phrase of phrases) {
    const matched = analyzeDeterministically(phrase.matchPhrase, catalog, input.locale)
    ...
    //
    // Explaining the phrase in pieces is the same failure wearing a better
    // disguise: it leaves no leftover text, so the leftover test above passes
    // while the dish has still been taken apart. Both fragment shapes have to
    // fail here or the model's one-item answer is undone on the way back.
    const fullyExplained = matched.items.length > 0 &&
      matched.unmatchedText.length === 0 &&
      !matched.fragmentedPhrase
    if (fullyExplained) {
...
async function cachedCandidateSelection(
  ...
  const model = process.env.OPENAI_SELECTION_MODEL?.trim() || 'gpt-5.4-nano'
  ...
  const secretSalt = process.env.SUPABASE_SERVICE_ROLE_KEY?.trim() || 'local-development'
