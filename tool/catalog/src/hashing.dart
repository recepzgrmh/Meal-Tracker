// Minimal SHA-256 so the catalog tools stay dependency-free.
//
// Shared by build_pilot_manifest.dart (snapshot verification) and
// fetch_sources.dart (hashing newly fetched pages). Kept byte-identical to the
// implementation that produced the hashes already recorded in
// snapshots/snapshot_index.json.
import 'dart:io';

String sha256OfFile(String path) => sha256OfBytes(File(path).readAsBytesSync());

String sha256OfBytes(List<int> message) {
  const k = <int>[
    0x428a2f98,
    0x71374491,
    0xb5c0fbcf,
