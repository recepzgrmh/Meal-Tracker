...
const envPath = join(fileURLToPath(new URL('../../', import.meta.url)), 'backend/.env')
...
async function pushTelemetryToSupabase() {
  console.log(`Pushing 7-day realistic telemetry to live Supabase DB (${supabaseUrl})...`)
    ...
    if (status !== 'failed' && i % 10 < 7 && queryObj.foods.length > 0) {
      ...
      queryObj.foods.forEach((foodId, idx) => {
        ...
        mealItems.push({
          ...
          grams: finalGrams,
          calories: Math.round(cal100 * finalGrams / 100),
          protein: Math.round(prot100 * finalGrams / 100 * 10) / 10,
          carbs: Math.round(carb100 * finalGrams / 100 * 10) / 10,
          fat: Math.round(fat100 * finalGrams / 100 * 10) / 10,
  ...
  // Upsert in batches into Supabase
  console.log('Upserting analysis runs...')
  for (let i = 0; i < analysisRuns.length; i += 50) {
    const chunk = analysisRuns.slice(i, i + 50)
  ...
  for (let i = 0; i < mealItems.length; i += 50) {
    const chunk = mealItems.slice(i, i + 50)
  ...
  for (let i = 0; i < corrections.length; i += 50) {
    ...
    const { error } = await supabaseAdmin.from('meal_item_corrections').upsert(chunk)
  ...
  console.log('SUCCESS! All 7-day realistic telemetry pushed to live Supabase DB.')
}
