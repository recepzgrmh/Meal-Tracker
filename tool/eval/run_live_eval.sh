...
#   tool/eval/run_live_eval.sh                       # 26 bilingual text cases
...
backend_url="${EVAL_BACKEND_URL:-http://localhost:8080}"
...
echo "Running $dataset (max $max_cases cases) against $backend_url ..." >&2
cd "$repo_root/backend"
EVAL_BACKEND_URL="$backend_url" \
...
SUPABASE_SERVICE_ROLE_KEY="$SUPABASE_SERVICE_ROLE_KEY" \
  npm run eval:live -- "$dataset" --persist
